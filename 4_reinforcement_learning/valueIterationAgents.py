...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
          ...
          for state in self.mdp.getStates():
            for index, action in enumerate(self.mdp.getPossibleActions(state)):
              if index == 0:
                Qmax = self.computeQValueFromValues(state, action)
              else:
                Qmax = max(Qmax, self.computeQValueFromValues(state, action))
    ...
    def computeQValueFromValues(self, state, action):
        ...
        Q = 0
        for sap in stateAndProbs:
          Q += sap[1] * (self.mdp.getReward(state, action, sap[0]) + self.discount * self.values[sap[0]])
        return Q
    ...
    def computeActionFromValues(self, state):
        ...
        for action in actions:
          ...
          expectedUtility = 0
          for sap in stateAndProbs:
            expectedUtility += sap[1] * self.values[sap[0]]
          actionDict[action] = expectedUtility
        return actionDict.argMax()[0]
